...
for type in methods:
    for size in [400, 800, 1600, 3200]:
        dirs.append(f"{size}-{type}")

for dir in dirs:
    timestamps = [[] for _ in range(100)]
    for filename in os.listdir(f"logs/scalability-{dir}"):
        with open(os.path.join(f"logs/scalability-{dir}", filename), "r") as file:
            for line in file:
    ...
    with open(f"result/prototype-scalability-{dir}.csv", mode="w") as csvfile:
...
for size in [400, 800, 1600, 3200]:
    result = pd.DataFrame(np.zeros((size, len(methods))))
    plt.rcParams["font.family"] = "Times New Roman"
    plt.figure(figsize=(2.5, 1.5))
    for i, method in enumerate(methods):
        df = pd.read_csv(
            f"result/prototype-scalability-{size}-{method}.csv", header=None
        )
    ...
    result.to_csv(f"result/prototype-scalability-{size}.csv", header=None, index=False)
    plt.xlabel("Time/ms")
    ...
    plt.grid(True)
    plt.savefig(f"images/prototype-scalability-{size}.pdf", bbox_inches="tight")
    plt.close()
